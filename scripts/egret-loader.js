...
(function () {
    ...
    // Egret 게임 플레이어 div 체크
    function checkEgretPlayer() {
        const playerDiv = document.querySelector(".egret-player");
        if (!playerDiv) {
            throw new Error(
                "Egret player div not found. Please add a div with class 'egret-player' in index.html"
            );
        }
        console.log("[EgretLoader] Egret player div found in HTML");
        return playerDiv;
    }
    ...
    window.EgretLoader = {
        ...
        init: async function (gameId) {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Failed to fetch manifest.json: ${response.status}`
                    );
                }
                ...
                window.HI5CORE.onLoad = async function () {
                    console.log(
                        "[EgretLoader] HI5CORE.onLoad called - Starting game load"
                    );
                    ...
                    try {
                        // Egret 플레이어 div 체크
                        const playerDiv = checkEgretPlayer();

                        // initial 스크립트 로드 (Egret 프레임워크)
                        if (manifest.initial && manifest.initial.length > 0) {
                            console.log(
                                `[EgretLoader] Loading ${manifest.initial.length} initial scripts...`
                            );
                        ...
                        if (manifest.game && manifest.game.length > 0) {
                            console.log(
                                `[EgretLoader] Loading ${manifest.game.length} game scripts...`
                            );
                        ...
                        if (window.egret) {
                            ...
                            if (typeof window.egret.runEgret === "function") {
                                console.log(
                                    "[EgretLoader] Calling egret.runEgret()"
                                );
                                window.egret.runEgret({
                                    renderMode: "webgl",
                                    audioType: 2,
                                    calculateCanvasScaleFactor: function (
                                        context
                                    ) {
                                        var backingStore =
                                ...
                                console.log(
                                    '[EgretLoader] Egret will auto-start with data-entry-class="Main"'
                                );
                            }

                            console.log(
                                "[EgretLoader] Game started successfully"
                            );
                        } else {
                            throw new Error(
                                "window.egret is not available after loading scripts"
                            );
                        }
                    } catch (error) {
                        console.error(
                            "[EgretLoader] Failed to load game:",
                            error
                        );
                        // 사용자에게 에러 표시
                        if (
                            window.HI5CORE &&
                            typeof window.HI5CORE.showAlert === "function"
                        ) {
                            window.HI5CORE.showAlert(
                                "게임 로드 실패: " + error.message
                            );
                        }
                        throw error;
                    }
